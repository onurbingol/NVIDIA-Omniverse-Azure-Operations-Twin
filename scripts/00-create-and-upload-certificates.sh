...
for DOMAIN in "${array[@]}"; do
    certbot --config-dir $SCRIPT_PATH/../certificates --work-dir $SCRIPT_PATH/../certificates --logs-dir $SCRIPT_PATH/../certificates --manual --preferred-challenges dns certonly -d "*.$DOMAIN,$DOMAIN" -m $EMAIL --agree-tos
